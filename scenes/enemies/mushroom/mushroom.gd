...
@export var idle_distance_to_player = 40
@export var projectile : PackedScene
@export var wait_before_deactivate = 4
@export var knockback_force = 80
@export var knockback_decrease = 3
...
func _on_inactive_state_entered():
	_sprtie.play("Inactive")


func _on_activate_state_entered():
	_sprtie.play("Activate")
	await _sprtie.animation_finished
	_state.send_event("idle")


func _on_deactivate_state_entered():
	_sprtie.play("Deactivate")
	await _sprtie.animation_finished
...
func _physics_process(delta):
	...
	if Time.get_ticks_msec() - _active_time > wait_before_deactivate*1000:
		if (!_player_in_range):  # TODO: Add more indicators when to disable
	...
	if (state != "Inactive") and (state != "Deactiva") \
		...
		_sprtie.scale.x = 1 if (_player.position.x - self.position.x < 0) else -1
