...
func _on_chase_state_physics_processing(delta: float) -> void:
	...
	if _sprite.animation == "move90" or _sprite.animation == "move-90":
		_enemy.rotation -= PI/2
...
func _physics_process(delta: float) -> void:
	...
	if (state.left(4) != "Idle") and (state.left(4) != "Dead"):
		#Get angle
		var direction : float = (_enemy.global_position - _position).angle()
		...
		if !_sprite.is_playing():
			_sprite.play("move" + str(snaped_angle))
